...
interface Props {
  similarPost: Post[]; // Add array notation
}

function SimilarPostComp({ similarPost }: Props) {


// Remove the first element if the array exists because we are already displaying the element, that is the page.
const removedFirstElement = similarPost?.slice(1, similarPost.length) || [];


  return (
    <div className="flex flex-wrap gap-4 py-3">
      {removedFirstElement.map((obj) => (
      <div
        key={obj.id}
